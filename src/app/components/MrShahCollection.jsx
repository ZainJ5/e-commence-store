...
export default function MrShahCollection() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch Mr. Shah collection products');
        }
        ...
        console.error('Error fetching Mr. Shah collection products:', error);
        setError('Failed to load Mr. Shah collection products. Please try again later.');
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between mb-12">
          <div className="text-center lg:text-left">
            ...
            <p className="text-sm text-gray-600 font-light tracking-wide uppercase mb-4">
              Discover our exclusive premium selection
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-yellow-50 border border-yellow-200 text-yellow-700 px-6 py-4 rounded-lg text-center animate-fadeIn" role="alert">
            <span className="font-medium">No products available in the Mr. Shah Collection.</span>
          </div>
        ) : (
          <div className="products-scroll transition-all duration-300 ease-in-out opacity-100 transform translate-y-0">
            <style jsx>{`
  .products-scroll {
    display: flex;
    overflow-x: auto;
...
`}</style>
        ...
        {products.length > 0 && (
          <div className="text-center mt-10">
            <Link
              href="/collections/mr-shah-collection"
